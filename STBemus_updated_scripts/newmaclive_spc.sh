...
#Scadenza - stampata
echo "Scadenza: "
curl --compressed -s -A 'Mozilla/5.0 (QtEmbedded; U; Linux; C) AppleWebKit/533.3 (KHTML, like Gecko) MAG200 stbapp ver: 2 rev: 250 Safari/533.3' -e "http://${SERVER}:${PORT}/stalker_portal/c/" -b "mac=${MAC}; stb_lang=en; timezone=Europe/Paris" -H 'X-User-Agent: Model: MAG250;Link: WiFi' -H 'Cache-Control: no-cache' -H 'Connection: keep-alive' -H "Host: ${SERVER}:${PORT}" -H "Accept-Language: en-US,*" -H "Accept-Charset: UTF-8,*;q=0.8" -H 'Accept-Encoding: gzip' -H "Authorization: Bearer ${AUTH}" "http://${SERVER}:${PORT}/portal.php?type=account_info&action=get_main_info&JsHttpRequest=1-xml&mac=${MAC}" | jq '.[].phone' | tr -d '"'

#LINK
LINK=$(curl --compressed -s -A 'Mozilla/5.0 (QtEmbedded; U; Linux; C) AppleWebKit/533.3 (KHTML, like Gecko) MAG200 stbapp ver: 2 rev: 250 Safari/533.3' -e 'http://${SERVER}:${PORT}/stalker_portal/c/' -b "mac=${MAC}; stb_lang=en; timezone=Europe/Paris" -H 'X-User-Agent: Model: MAG250;Link: WiFi' -H 'Cache-Control: no-cache' -H 'Connection: keep-alive' -H "Host: ${SERVER}:${PORT}" -H "Accept-Language: en-US,*" -H "Accept-Charset: UTF-8,*;q=0.8" -H 'Accept-Encoding: gzip' -H "Authorization: Bearer ${AUTH}" "http://${SERVER}:${PORT}/portal.php?type=itv&action=create_link&forced_storage=undefined&download=0&cmd=ffmpeg http://localhost/ch/${CHANNEL}_" | jq -r '.[].cmd' | tr -d '"' | cut -c8-)
echo $LINK
